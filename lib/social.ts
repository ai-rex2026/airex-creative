 ...
 *
 * x_search には「ポスト検索」と「ユーザープロフィール取得」の2種類の動き方があり、
 * フォロワー数のようなプロフィール上の数値は後者でないとほぼ返ってこない。
 * 以前はポスト検索のクエリだけを指示していたため、投稿は見つかっても
 * フォロワー数だけ取得できない（found:true・followers:null）ケースが多かった。
 * そのため、まずプロフィールそのものを取得するよう明示し、それが失敗した
 * ときだけポスト検索にフォールバックする指示に変えている。
 *
...
async function readX(a: { platform: string; url: string; handle: string }, base: SocialAccount): Promise<SocialAccount> {
  ...
  const prompt = `x_search ツールで、Xのアカウント「@${a.handle}」のユーザープロフィールそのものを調べてください
（allowed_x_handles で対象は既にこのアカウント1件に絞ってあります）。ポストの検索ではなく、
このアカウントのプロフィールページに表示されている情報（フォロワー数・フォロー数・投稿数など）を
直接取得することを優先してください。プロフィールの取得がうまくいかない場合のみ、
検索クエリに「${a.handle}」や「${a.handle} profile」を使ってポストを検索し、そこから分かる範囲で補ってください。

取得できたプロフィール情報（無ければポストの情報）から、次を答えてください。
