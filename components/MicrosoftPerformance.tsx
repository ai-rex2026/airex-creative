...
import { microsoftPerformance, type MicrosoftAdPerformance as Perf } from "@/app/ad-performance-actions";
...
/** 選んだ Microsoft 広告アカウントの、キャンペーン別の実績（読み取りのみ）。期間は初期値が直近30日 */
export function MicrosoftPerformance() {
  ...
  const load = () => startTransition(async () => setData(await microsoftPerformance(from, to)));

  return (
    <section style={{ margin: "28px auto 0", maxWidth: 960, padding: "0 16px" }}>
      <h2 style={{ fontSize: 16, color: "var(--head)", margin: "0 0 10px" }}>広告実績（Microsoft 広告・キャンペーン別）</h2>
      ...
      <p style={{ fontSize: 11.5, color: "var(--faint)" }}>
        読み取りのみ。広告アカウントの設定は変更しません。金額は各アカウントの通貨のままの数値です。レポート生成が非同期のため、
        Google 広告・ヤフーLINE広告より数十秒ほど時間がかかることがあります。
      </p>
    </section>
  );
}
