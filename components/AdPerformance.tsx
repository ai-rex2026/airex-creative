...
import { googlePerformance, type AdPerformance as Perf } from "@/app/ad-performance-actions";
...
const STATUS: Record<string, string> = { ENABLED: "有効", PAUSED: "停止中", REMOVED: "削除" };
...
/** 選んだ Google 広告アカウントの、キャンペーン別の実績（読み取りのみ）。期間は初期値が直近30日 */
export function AdPerformance() {
  ...
  const load = () => startTransition(async () => setData(await googlePerformance(from, to)));

  return (
    <section style={{ margin: "28px auto 0", maxWidth: 960, padding: "0 16px" }}>
      <h2 style={{ fontSize: 16, color: "var(--head)", margin: "0 0 10px" }}>広告実績（Google 広告・キャンペーン別）</h2>
                        ...
                        <tr key={c.id} style={{ borderBottom: "1px solid var(--line)" }}>
                          <td style={{ ...CELL, textAlign: "left", whiteSpace: "normal" }}>
                            ...
                            {c.status && c.status !== "ENABLED" && (
                              <span style={{ color: "var(--faint)", marginLeft: 6, fontSize: 11 }}>{STATUS[c.status] ?? c.status}</span>
                            )}
                          </td>
      ...
      <p style={{ fontSize: 11.5, color: "var(--faint)" }}>読み取りのみ。広告アカウントの設定は変更しません。金額は各アカウントの通貨のままの数値です。</p>
    </section>
  );
}
